...
class TrackingEnv:
    ...
    def __init__(
        ...
        self._config = config
        self._useObstacles = (
            config.get("useObstacles") if useObstacles is None else useObstacles
        )
        ...
        self.Ts = config["Ts"]
        self.lidarModel = OptimizedLidarModel(
            120, 1, 10
        )  # 120 degrees, 0.5 meters range, 10 points
    ...
    def reset(self, seed=None):
        ...
        self.car = BicycleModel(
            self.referencePath, self.carLength, self.carWidth, self.Ts
        )
        self._stepCount = 0
        self._lastVelocity = 0.0
    ...
    def step(self, action):
        ...
        if not lapComplete:
            ...
            self.car.spatial_state = self.car.t2s(
                self.car.current_waypoint, self.car.temporal_state
            )
        ...
        reward = self.computeReward(
            velocity,
            lateralError,
            self.car.spatial_state.e_psi,
            collision,
            self.car.s - self._lastS,
            self.lidarModel.measurements[1, :],
            lapComplete,
        )
    ...
    def render(self):
        ...
        self.car.show()
        plt.title(
            f"RL Environment  |  s={self.car.s:.2f}/{self.referencePath.length:.2f} m  |  lateralError={self.car.spatial_state.e_y:.3f} m"
        )
        plt.axis("off")
    ...
    def getObs(self):
        ...
        rightBound = self.car.current_waypoint.ub
        clearance = min(
            rightBound - lateralError, lateralError - leftBound
        )  # distance to the closer boundary
    ...
    def computeReward(
        ...
        #        obstacle_penalty = (
        #            -2.0 * (safe_distance - nearest_obstacle) / safe_distance
        #        )
