...
totalSteps = 150000  # lowered training steps (for now) but can be increased later
...
checkpointFreq = 50000  # save weights every N steps once past the early phase
...
earlyStopEvals = 3  # stop after this many consecutive perfect evals
...
def train(useObstacles=True):
    os.makedirs(checkpointDir, exist_ok=True)

    # clear out checkpoints from a previous run so the plot only covers this run,
    # otherwise old later-step files make it look like training ran the full length
    for old in glob.glob(os.path.join(checkpointDir, 'sac_*.pt')):
        os.remove(old)

    env = TrackingEnv(simMode='Sim_Track', useObstacles=useObstacles)
    # separate env for evaluation so it never disturbs the training state
    ...
    buffer = ReplayBuffer(state_dim=5, action_dim=2, capacity=bufferCapacity)
    ...
    for step in range(1, totalSteps + 1):
        ...
        # periodic evaluation with an early stop once the agent laps cleanly a few times to not run full training (we can remove this later if needed)
        if step % evalFreq == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-obstacles', action='store_true',
                        help='train without obstacles on the track')
    args = parser.parse_args()
    train(useObstacles=not args.no_obstacles)
